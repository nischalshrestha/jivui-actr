(function() {
  ...
  Timeline.onDataProcessed = function(settings, data) {
    ...
    for(var ms in data) {
      ...
      // new entry types to accomodate concentration viz; type is an attribue in JSON
      // conflict resolution (act-r)
  ...
  function TimelineObject() {
    return {
      ...
      // modified above function to better represent a cognitive event in act-r
      // and make it more general with attribute "type"
      addCognitive: function(ms, cognitive) {
      ...
      addClick: function(ms, click) {
        ...
        tooltip += "<br>Point: ("+click.x+", "+click.y+")"
        tooltip += "<br>Row: "  + click.row + " Col: "+click.col;
        newItem.tooltip({
          title: tooltip,
          html: true,
          container: "body"
      ...
      // new: represents a mouse movement event
      ...
      // modified: save as above except for perceptual.type (we might have different types not just word)
      addPerceptual: function(ms, perceptual) {
